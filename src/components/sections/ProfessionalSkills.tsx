...
function SkillCard({ skill, index, delay }: {
  ...
  return (
    <motion.div
      ...
      className={`group p-5 rounded-xl backdrop-blur-sm border transition-all duration-300 ${theme === 'dark'
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            <div className={`h-1.5 w-full max-w-[140px] rounded-full overflow-hidden ${theme === 'dark' ? 'bg-dark-600/70' : 'bg-gray-200'
              }`}>
              <motion.div
        ...
        >
          <svg
            ...
            className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''} ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
              }`}
          >
      ...
      >
        ...
        {skill.projects && skill.projects.length > 0 && (
          <div className="mb-3">
            <h5 className={`text-xs font-semibold uppercase mb-1.5 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
              }`}>
              Related Projects
            </h5>
            <div className="flex flex-wrap gap-2">
              {skill.projects.map(project => (
                <span
                  key={project}
                  className={`text-xs px-2 py-1 rounded-md ${theme === 'dark'
    ...
    >
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <div className="h-0.5 w-16 bg-gradient-to-r from-primary-500 to-accent-cyan mt-1" />
        </div>
      </div>
  ...
  return (
    <section id="skills" className="section-padding relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className={`absolute top-1/4 left-1/6 w-96 h-96 rounded-full opacity-5 ${theme === 'dark' ? 'bg-blue-500' : 'bg-blue-400'
          } blur-3xl`} />
        <div className={`absolute bottom-1/4 right-1/6 w-80 h-80 rounded-full opacity-5 ${theme === 'dark' ? 'bg-purple-500' : 'bg-purple-400'
          } blur-3xl`} />
      </div>
      ...
      <div className="container-custom relative z-10">
        ...
        >
          <h2 className={`text-4xl lg:text-5xl font-bold mb-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'
            ...
            Professional <span className="bg-gradient-to-r from-primary-400 via-accent-cyan to-primary-400 bg-clip-text text-transparent">Expertise</span>
          </h2>

          <p className={`text-lg max-w-3xl mx-auto leading-relaxed ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
            }`}>
        ...
        >
          <div className={`p-5 sm:p-8 rounded-xl backdrop-blur-sm border ${theme === 'dark'
              ? 'bg-dark-800/50 border-gray-700/30'
              : 'bg-white/80 border-gray-200/60'
            }`}>
            <div className="flex flex-col sm:flex-row items-start gap-5">
              <div className={`p-3 rounded-xl shrink-0 mb-3 sm:mb-0 ${theme === 'dark'
                ...
                <ul className="grid gap-3 grid-cols-1 sm:grid-cols-2">
                  {[
                    ...
                    <li key={index} className="flex gap-3 mb-3 sm:mb-0">
                      <div className={`mt-1 shrink-0 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
                        }`}>
                        <Workflow size={16} />
                      </div>
                      <div>
                        <h4 className={`text-sm font-semibold mb-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'
                          }`}>
                          {item.title}
                        </h4>
